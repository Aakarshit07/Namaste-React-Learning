...
const resList =  [
  ...
  availability: {
  nextCloseTime: "2024-01-29 00:59:00",
  opened: true
  },
  badges: { },
  isOpen: true,
...
const RestaurantCard = (props) => {
  ...
  return (
      <div className="res-card" style={{backgroundColor: "rgba(251, 144, 83, 0.491)"}} >
        <img  className="res-img" src={"https://media-assets.swiggy.com/swiggy/image/upload/fl_lossy,f_auto,q_auto,w_1024/"+cloudinaryImageId}/>
        <h3>{name}</h3>
        <h4>{cuisines.join(", ")}</h4>
        <h4>{costForTwo}</h4>
  ...
  return (
    <div className="body">
      ...
      <div className="res-container">
        {
          resList.map((card) => <RestaurantCard key={card.info.id} resData={card} /> )
        }  
      </div>
    </div>
  )
